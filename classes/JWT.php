...
class JWT
{
    private static $key = "myglobalkey"; // Set the global key here
    ...
    public static function decode($jwt, $algo = 'HS256')
    {
        $tokens = explode('.', $jwt);
        if (count($tokens) != 3) {
        ...
        $signatureCheck = hash_hmac('sha256', "$header64.$payload64", self::$key, true);
